...
class Form extends Component
{
    ...
    public function updatedPegawaiId($value)
    {
        ...
        $this->detail = collect(collect($this->dataPegawai)->where('id', $value)->first()['kepegawaian_pegawai_unsur_gaji'])->map(fn($q) => [
            ...
            'debet' => $q['sifat'] == '+' ? $q['nilai'] : 0,
            'kredit' => $q['sifat'] == '-' ? $q['nilai'] : 0,
            'sifat' => $q['sifat'],
        ])->toArray();
    }
    ...
    public function submit()
    {
        ...
        DB::transaction(function () {
            ...
            $this->detail[] = [
                ...
                'kode_akun_nama' => collect($this->dataKodeAkun)->where('id', $this->metode_bayar)->first()['nama'],
                'debet' => 0,
                'kredit' => collect($this->detail)->sum('debet') - collect($this->detail)->sum('kredit'),
            ];
