<table class="table table-hover">
    <thead>
        <tr>
            ...
            <th rowspan="2" class="text-end">Harga Beli Terakhir</th>
    ...
    <tbody>
        @foreach ($data as $item)
            <tr>
                ...
                <td class="text-end">
                    {{ $cetak == false ? number_format_id($item->barang->getHargaBeliTertinggi($item->rasio_dari_terkecil)) : $item->barang->getHargaBeliTertinggi($item->rasio_dari_terkecil) }}
                </td>
                @php
                    $keuntungan = $item->harga_jual - $item->barang->getHargaBeliTertinggi($item->rasio_dari_terkecil);
                @endphp
